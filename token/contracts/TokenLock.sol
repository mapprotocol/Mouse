...
contract User {
    ...
    function register (string memory _id, string memory _nation, string memory _name, string memory _address) public {
        Identity storage info =  userInfo[msg.sender];
        
        info.id = _id;
        info.nation = _nation;
        info.name = _name;
        info.userAddress = _address;
    }
...
contract Pack {
    constructor(address _from, address _to, uint256 _value, bytes32 _tx, string memory _id, string memory _nation, string memory _name, string memory _address, bytes memory proof) public {
    }
}
