...
func makeMiner(genesis *core.Genesis) (*node.Node, *mos.Mouse, error) {
	...
	// Create the node and configure a full Mouse node on it
	stack, err := node.New(config)
	if err != nil {
		return nil, nil, err
	}
	ethBackend, err := mos.New(stack, &mos.Config{
		Genesis:         genesis,
		...
		GPO:             mos.DefaultConfig.GPO,
		Ethash:          mos.DefaultConfig.Ethash,
		Miner: miner.Config{
