...
public class AndroidTest extends InstrumentationTestCase {
	...
	public void testAccountManagement() {
		...
		KeyStore ks = new KeyStore(getInstrumentation().getContext().getFilesDir() + "/keystore", Gmos.LightScryptN, Gmos.LightScryptP);
	...
	public void testInprocNode() {
		...
		try {
			...
			Node node = new Node(getInstrumentation().getContext().getFilesDir() + "/.mouse", new NodeConfig());
			node.start();
			...
			MouseClient ec = node.getEthereumClient();
	...
	// signatures too. Regression test for mouse issue #14599.
...
func TestAndroid(t *testing.T) {
	...
	workspace, err := ioutil.TempDir("", "gmos-android-")
	if err != nil {
	...
	for _, dir := range []string{"src/main", "src/androidTest/java/org/mouse/gethtest", "libs"} {
		err = os.MkdirAll(dir, os.ModePerm)
		if err != nil {
			t.Fatal(err)
		}
	}
	// Generate the mobile bindings for Gmos and add the tester class
	gobind := exec.Command("gomobile", "bind", "-javapkg", "org.mouse", "github.com/marcopoloprotoco/mouse/mobile")
	if output, err := gobind.CombinedOutput(); err != nil {
		t.Logf("%s", output)
		t.Fatalf("failed to run gomobile bind: %v", err)
	}
	cp.CopyFile(filepath.Join("libs", "gmos.aar"), "gmos.aar")

	if err = ioutil.WriteFile(filepath.Join("src", "androidTest", "java", "org", "mouse", "gethtest", "AndroidTest.java"), []byte(androidTestClass), os.ModePerm); err != nil {
