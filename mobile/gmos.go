...
func NewNode(datadir string, config *NodeConfig) (stack *Node, _ error) {
	...
	if config.EthereumEnabled {
		...
		if err != nil {
			return nil, fmt.Errorf("mouse init: %v", err)
		}
		// If netstats reporting is requested, do it
		if config.EthereumNetStats != "" {
			if err := mosstats.New(rawStack, lesBackend.ApiBackend, lesBackend.Engine(), config.EthereumNetStats); err != nil {
...
func (n *Node) GetEthereumClient() (client *MouseClient, _ error) {
	...
	return &MouseClient{mosclient.NewClient(rpc)}, nil
}
