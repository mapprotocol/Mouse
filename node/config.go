...
import (
	...
	"github.com/marcopoloprotoco/mouse/accounts/external"
	"github.com/marcopoloprotoco/mouse/accounts/keystore"
	"github.com/marcopoloprotoco/mouse/accounts/scwallet"
	"github.com/marcopoloprotoco/mouse/accounts/usbwallet"
...
type Config struct {
	...
	// used in the devp2p node identifier. The instance name of gmos is "gmos". If no
...
// These resources are resolved differently for "gmos" instances.
...
func (c *Config) ResolvePath(path string) string {
	...
	// by gmos 1.4 are used if they exist.
	if warn, isOld := isOldGethResource[path]; isOld {
		...
		if oldpath != "" && common.FileExist(oldpath) {
			if warn {
				c.warnOnce(&c.oldGethResourceWarning, "Using deprecated resource file %s, please move this file to the 'gmos' subdirectory of datadir.", oldpath)
			}
			return oldpath
		}
	}
...
func makeAccountManager(conf *Config) (*accounts.Manager, string, error) {
	...
	if keydir == "" {
		// There is no datadir.
		keydir, err = ioutil.TempDir("", "mouse-keystore")
		ephemeral = keydir
	}
