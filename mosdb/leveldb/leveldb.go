...
func (db *Database) NewBatch() mosdb.Batch {
	return &batch{
		db: db.db,
		b:  new(leveldb.Batch),
	}
...
func (db *Database) NewIterator(prefix []byte, start []byte) mosdb.Iterator {
...
func (b *batch) Replay(w mosdb.KeyValueWriter) error {
	return b.b.Replay(&replayer{writer: w})
}

// replayer is a small wrapper to implement the correct replay methods.
type replayer struct {
	writer  mosdb.KeyValueWriter
	failure error
}
