...
var (
	...
	retestethCommand = cli.Command{
		...
		Name:        "retesteth",
		Usage:       "Launches gmos in retesteth mode",
		ArgsUsage:   "",
		Flags:       []cli.Flag{rpcPortFlag},
		Category:    "MISCELLANEOUS COMMANDS",
		Description: `Launches gmos in retesteth mode (no database, no network, only retesteth RPC interface)`,
	}
)
...
func retesteth(ctx *cli.Context) error {
	...
	srv := rpc.NewServer()
	err := node.RegisterApisFromWhitelist(rpcAPI, []string{"test", "mos", "debug", "web3"}, srv, false)
	if err != nil {
