...
func makecache(ctx *cli.Context) error {
	args := ctx.Args()
	if len(args) != 2 {
		utils.Fatalf(`Usage: gmos makecache <block number> <outputdir>`)
	}
...
func makedag(ctx *cli.Context) error {
	args := ctx.Args()
	if len(args) != 2 {
		utils.Fatalf(`Usage: gmos makedag <block number> <outputdir>`)
	}
	block, err := strconv.ParseUint(args[0], 0, 64)
	if err != nil {
		utils.Fatalf("Invalid block number: %v", err)
...
func version(ctx *cli.Context) error {
	...
	fmt.Println("Protocol Versions:", mos.ProtocolVersions)
...
func license(_ *cli.Context) error {
	fmt.Println(`Gmos is free software: you can redistribute it and/or modify
it under the terms of the GNU General Public License as published by
the Free Software Foundation, either version 3 of the License, or
(at your option) any later version.
...
You should have received a copy of the GNU General Public License
along with gmos. If not, see <http://www.gnu.org/licenses/>.`)
	return nil
}
