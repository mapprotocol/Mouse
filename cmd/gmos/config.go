...
func defaultNodeConfig() node.Config {
	...
	cfg.HTTPModules = append(cfg.HTTPModules, "mos")
	cfg.WSModules = append(cfg.WSModules, "mos")
	cfg.IPCPath = "gmos.ipc"
	return cfg
}

// makeConfigNode loads gmos configuration and creates a blank node instance.
func makeConfigNode(ctx *cli.Context) (*node.Node, gethConfig) {
	// Load defaults.
	cfg := gethConfig{
		Eth:  mos.DefaultConfig,
	...
	if file := ctx.GlobalString(configFileFlag.Name); file != "" {
		...
		if cfg.Shh != (whisperDeprecatedConfig{}) {
			log.Warn("Deprecated whisper config detected. Whisper has been moved to github.com/marcopoloprotoco/whisper")
		}
	}
...
func checkWhisper(ctx *cli.Context) {
	for _, flag := range whisperFlags {
		if ctx.GlobalIsSet(flag.GetName()) {
			log.Warn("deprecated whisper flag detected.")
		}
	}
}
