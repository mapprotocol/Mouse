...
var (
	...
	netFlag     = flag.Uint64("network", 0, "Network ID to use for the Mouse protocol")
	statsFlag   = flag.String("mosstats", "", "Ethstats network monitoring auth string")
	...
	logFlag    = flag.Int("loglevel", 3, "Log level to use for Mouse and the faucet")
)
...
type request struct {
	...
	Account common.Address     `json:"account"` // Mouse address being funded
	Time    time.Time          `json:"time"`    // Timestamp when the request was accepted
	Tx      *types.Transaction `json:"tx"`      // Transaction funding the account
}

// faucet represents a crypto faucet backed by an Mouse light client.
type faucet struct {
	config *params.ChainConfig // Chain configurations for signing
	stack  *node.Node          // Mouse protocol stack
	client *mosclient.Client   // Client connection to the Mouse chain
...
func newFaucet(genesis *core.Genesis, port int, enodes []*discv5.Node, network uint64, stats string, ks *keystore.KeyStore, index []byte) (*faucet, error) {
	...
	// Assemble the Mouse light client protocol
	...
	if err != nil {
		return nil, fmt.Errorf("Failed to register the Mouse service: %w", err)
	}

	// Assemble the mosstats monitoring and reporting service'
	if stats != "" {
		if err := mosstats.New(stack, lesBackend.ApiBackend, lesBackend.Engine(), stats); err != nil {
			return nil, err
		}
	}
	...
	client := mosclient.NewClient(api)
...
// close terminates the Mouse connection and tears down the faucet.
...
func (f *faucet) apiHandler(w http.ResponseWriter, r *http.Request) {
	...
	for {
		...
		// Retrieve the Mouse address to fund, the requesting user and a profile picture
		var (
			username string
			avatar   string
		...
		default:
			//lint:ignore ST1005 This error is to be displayed in the browser
			err = errors.New("Something funky happened, please open an issue at https://github.com/marcopoloprotoco/mouse/issues")
		}
		if err != nil {
			if err = sendError(conn, err); err != nil {
...
// the username, avatar URL and Mouse address to fund on success.
func authTwitter(url string) (string, string, common.Address, error) {
	...
	// and scrape it for the Mouse address and profile URL. We need to load
	...
	if address == (common.Address{}) {
		//lint:ignore ST1005 This error is to be displayed in the browser
		return "", "", common.Address{}, errors.New("No Mouse address found to fund")
	}
	var avatar string
...
// returning the username, avatar URL and Mouse address to fund on success.
func authFacebook(url string) (string, string, common.Address, error) {
	...
	// scrape it for the Mouse address and profile URL.
	res, err := http.Get(url)
	if err != nil {
		return "", "", common.Address{}, err
	}
...
// authNoAuth tries to interpret a faucet request as a plain Mouse address,
