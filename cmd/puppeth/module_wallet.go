...
RUN \
  echo 'node server.js &'                     > wallet.sh && \
	echo 'gmos --cache 512 init /genesis.json' >> wallet.sh && \
	echo $'exec gmos --networkid {{.NetworkID}} --port {{.NodePort}} --bootnodes {{.Bootnodes}} --mosstats \'{{.Ethstats}}\' --cache=512 --http --http.addr=0.0.0.0 --http.corsdomain "*" --http.vhosts "*"' >> wallet.sh
