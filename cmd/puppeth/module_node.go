...
// nodeDockerfile is the Dockerfile required to run an Mouse node.
var nodeDockerfile = `
FROM mouse/client-go:latest
...
{{end}}
RUN \
  echo 'gmos --cache 512 init /genesis.json' > gmos.sh && \{{if .Unlock}}
	echo 'mkdir -p /root/.mouse/keystore/ && cp /signer.json /root/.mouse/keystore/' >> gmos.sh && \{{end}}
	echo $'exec gmos --networkid {{.NetworkID}} --cache 512 --port {{.Port}} --nat extip:{{.IP}} --maxpeers {{.Peers}} {{.LightFlag}} --mosstats \'{{.Ethstats}}\' {{if .Bootnodes}}--bootnodes {{.Bootnodes}}{{end}} {{if .Etherbase}}--miner.etherbase {{.Etherbase}} --mine --miner.threads 1{{end}} {{if .Unlock}}--unlock 0 --password /signer.pass --mine{{end}} --miner.gastarget {{.GasTarget}} --miner.gaslimit {{.GasLimit}} --miner.gasprice {{.GasPrice}}' >> gmos.sh

ENTRYPOINT ["/bin/sh", "gmos.sh"]
`

// nodeComposefile is the docker-compose.yml file required to deploy and maintain
// an Mouse node (bootnode or miner for now).
var nodeComposefile = `
version: '2'
services:
  {{.Type}}:
    ...
    ports:
      ...
      - "{{.Port}}:{{.Port}}/udp"
    volumes:
      - {{.Datadir}}:/root/.mouse{{if .Ethashdir}}
...
// deployNode deploys a new Mouse node container to a remote machine via SSH,
...
func deployNode(client *sshClient, network string, bootnodes []string, config *nodeInfos, nocache bool) ([]byte, error) {
	...
	template.Must(template.New("").Parse(nodeComposefile)).Execute(composefile, map[string]interface{}{
		...
		"Ethstats":   config.mosstats[:strings.Index(config.mosstats, ":")],
		"Etherbase":  config.etherbase,
...
func (info *nodeInfos) Report() map[string]string {
	report := map[string]string{
		...
		"Ethstats username":        info.mosstats,
	}
	if info.gasTarget > 0 {
...
func checkNode(client *sshClient, network string, boot bool) (*nodeInfos, error) {
	...
	var out []byte
	if out, err = client.Run(fmt.Sprintf("docker exec %s_%s_1 gmos --exec admin.nodeInfo.enode --cache=16 attach", network, kind)); err != nil {
	...
	stats := &nodeInfos{
		genesis:    genesis,
		datadir:    infos.volumes["/root/.mouse"],
		ethashdir:  infos.volumes["/root/.ethash"],
		port:       port,
		peersTotal: totalPeers,
		peersLight: lightPeers,
		mosstats:   infos.envvars["STATS_NAME"],
