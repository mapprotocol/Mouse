...
func (w *wizard) deployNode(boot bool) {
	...
	if infos.mosstats == "" {
		...
		infos.mosstats = w.readString() + ":" + w.conf.mosstats
	} else {
		fmt.Printf("What should the node be called on the stats page? (default = %s)\n", infos.mosstats)
		infos.mosstats = w.readDefaultString(infos.mosstats) + ":" + w.conf.mosstats
	}
	...
	if out, err := deployNode(client, w.network, w.conf.bootnodes, infos, nocache); err != nil {
		log.Error("Failed to deploy Mouse node container", "err", err)
		if len(out) > 0 {
			fmt.Printf("%s\n", out)
		}
		return
