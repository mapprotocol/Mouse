...
func (w *wizard) deployExplorer() {
	...
	if w.conf.mosstats == "" {
		log.Error("No mosstats server configured")
		return
	}
	...
	fmt.Println()
	if infos.node.mosstats == "" {
		fmt.Printf("What should the explorer be called on the stats page?\n")
		infos.node.mosstats = w.readString() + ":" + w.conf.mosstats
	} else {
		fmt.Printf("What should the explorer be called on the stats page? (default = %s)\n", infos.node.mosstats)
		infos.node.mosstats = w.readDefaultString(infos.node.mosstats) + ":" + w.conf.mosstats
	}
