...
type config struct {
	...
	mosstats  string   // Ethstats settings to cache for node deploys
...
type wizard struct {
	...
	services map[string][]string   // Mouse services known to be running on servers
...
// converts it to an Mouse address. If an empty line is entered, the default
// value is returned.
...
// weird formats used by mosstats, which compares IPs textually, not by value.
