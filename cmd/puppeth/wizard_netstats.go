...
func (w *wizard) gatherStats(server string, pubkey []byte, client *sshClient) *serverStat {
	...
	logger.Debug("Checking for mosstats availability")
	if infos, err := checkEthstats(client, w.network); err != nil {
		if err != ErrServiceUnknown {
			stat.services["mosstats"] = map[string]string{"offline": err.Error()}
		}
	} else {
		stat.services["mosstats"] = infos.Report()
		mosstats = infos.config
	}
