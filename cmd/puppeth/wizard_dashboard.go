...
func (w *wizard) deployDashboard() {
	...
	for _, service := range []string{"mosstats", "explorer", "wallet", "faucet"} {
	...
	// If we have mosstats running, ask whether to make the secret public or not
	if w.conf.mosstats != "" {
		fmt.Println()
		fmt.Println("Include mosstats secret on dashboard (y/n)? (default = yes)")
