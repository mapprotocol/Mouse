...
RUN \
  echo 'gmos --cache 512 init /genesis.json' > explorer.sh && \
  echo $'gmos --networkid {{.NetworkID}} --syncmode "full" --gcmode "archive" --port {{.EthPort}} --bootnodes {{.Bootnodes}} --mosstats \'{{.Ethstats}}\' --cache=512 --http --http.api "net,web3,mos,shh,debug" --http.corsdomain "*" --http.vhosts "*" --ws --ws.origins "*" --exitwhensynced' >> explorer.sh && \
  echo $'exec gmos --networkid {{.NetworkID}} --syncmode "full" --gcmode "archive" --port {{.EthPort}} --bootnodes {{.Bootnodes}} --mosstats \'{{.Ethstats}}\' --cache=512 --http --http.api "net,web3,mos,shh,debug" --http.corsdomain "*" --http.vhosts "*" --ws --ws.origins "*" &' >> explorer.sh && \
...
func deployExplorer(client *sshClient, network string, bootnodes []string, config *explorerInfos, nocache bool, isClique bool) ([]byte, error) {
	...
	template.Must(template.New("").Parse(explorerComposefile)).Execute(composefile, map[string]interface{}{
		...
		"DBDir":       config.dbdir,
		"EthPort":     config.node.port,
		"EthName":     config.node.mosstats[:strings.Index(config.node.mosstats, ":")],
		"WebPort":     config.port,
...
func checkExplorer(client *sshClient, network string) (*explorerInfos, error) {
	...
	stats := &explorerInfos{
		node: &nodeInfos{
			datadir:  infos.volumes["/opt/app/.mouse"],
			port:     infos.portmap[infos.envvars["ETH_PORT"]+"/tcp"],
			mosstats: infos.envvars["ETH_NAME"],
		},
