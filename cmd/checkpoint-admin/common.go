...
import (
	...
	"github.com/marcopoloprotoco/mouse/contracts/checkpointoracle"
...
func newClient(ctx *cli.Context) *mosclient.Client {
	client, err := mosclient.Dial(ctx.GlobalString(nodeURLFlag.Name))
	if err != nil {
		utils.Fatalf("Failed to connect to Mouse node: %v", err)
	}
	return client
}
...
func newContract(client *rpc.Client) (common.Address, *checkpointoracle.CheckpointOracle) {
	...
	contract, err := checkpointoracle.NewCheckpointOracle(addr, mosclient.NewClient(client))
	if err != nil {
