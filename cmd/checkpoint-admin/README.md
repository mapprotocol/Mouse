...
```shell
go get github.com/marcopoloprotoco/mouse/cmd/checkpoint-admin
```
...
It is worth noting that checkpoint-admin only supports clef as a signer for transactions and plain text(checkpoint). For more clef usage, please see the clef [tutorial](https://gmos.ethereum.org/docs/clef/tutorial) .
...
*CHECKPOINT_HASH is obtained based on this [calculation method](https://github.com/marcopoloprotoco/mouse/blob/master/params/config.go#L251).*
...
* Start gmos with the modified configuration file
