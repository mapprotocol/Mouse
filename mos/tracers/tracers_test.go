...
import (
	...
	"github.com/marcopoloprotoco/mouse/tests"
)

// To generate a new callTracer test, copy paste the makeTest method below into
// a Gmos console and call it with a transaction hash you which to export.
...
var makeTest = function(tx, rewind) {
  ...
  var block   = mos.getBlock(mos.getTransaction(tx).blockHash);
  var genesis = mos.getBlock(block.parentHash);
  ...
  genesis.config = admin.nodeInfo.protocols.mos.config;
  ...
  console.log(JSON.stringify({
    ...
    input:  mos.getRawTransaction(tx),
    result: result,
  }, null, 2));
}
*/
