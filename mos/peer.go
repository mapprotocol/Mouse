...
// PeerInfo represents a short summary of the Mouse sub-protocol metadata known
// about a connected peer.
type PeerInfo struct {
	Version    int      `json:"version"`    // Mouse protocol version negotiated
...
//
// This method is legacy support for initial transaction exchange in mos/64 and
// prior. For mos/65 and higher use SendPooledTransactionHashes.
...
func (p *peer) SendMMRReceiptProof(mtProof *ulvp.SimpleUlvpProof) error {
	return p2p.Send(p.rw, MMRReceiptProofMsg, mtProof)
}
...
// Handshake executes the mos protocol handshake, negotiating version number,
...
// network IDs, difficulties, head and genesis blocks.
func (p *peer) OtherHandshake(network uint64, td *big.Int, head common.Hash, genesis common.Hash, uLVP *core.SimpleULVP) error {
	// Send out own handshake in a new thread
	errc := make(chan error, 2)
...
func (p *peer) String() string {
	return fmt.Sprintf("Peer %s [%s]", p.id,
		fmt.Sprintf("mos/%2d", p.version),
	)
}

// peerSet represents the collection of active peers currently participating in
// the Mouse sub-protocol.
type peerSet struct {
