...
var (
	...
	maxQueuedHeaders            = 32 * 1024                         // [mos/62] Maximum number of headers to queue for import (DOS protection)
...
type Downloader struct {
	...
	stateDB    mosdb.Database  // Database to state sync into (and deduplicate via)
	...
	// Channels
	headerCh      chan dataPack        // [mos/62] Channel receiving inbound block headers
	bodyCh        chan dataPack        // [mos/62] Channel receiving inbound block bodies
	receiptCh     chan dataPack        // [mos/63] Channel receiving inbound receipts
	bodyWakeCh    chan bool            // [mos/62] Channel to signal the block body fetcher of new tasks
	receiptWakeCh chan bool            // [mos/63] Channel to signal the receipt fetcher of new tasks
	headerProcCh  chan []*types.Header // [mos/62] Channel to feed the header processor new tasks

	// for stateFetcher
	stateSyncStart chan *stateSync
	trackStateReq  chan *stateReq
	stateCh        chan dataPack // [mos/63] Channel receiving inbound node state data
...
func New(checkpoint uint64, stateDb mosdb.Database, stateBloom *trie.SyncBloom, mux *event.TypeMux, chain BlockChain, lightchain LightChain, dropPeer peerDropFn) *Downloader {
	if lightchain == nil {
		lightchain = chain
	}
	dl := &Downloader{
...
// these are zero.
func (d *Downloader) Progress() mouse.SyncProgress {
...
func (d *Downloader) syncWithPeer(p *peerConnection, hash common.Hash, td *big.Int) (err error) {
	...
	log.Debug("Synchronising with the network", "peer", p.id, "mos", p.version, "head", hash, "td", td, "mode", mode)
