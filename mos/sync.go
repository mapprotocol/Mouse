...
func (pm *ProtocolManager) syncTransactions(p *peer) {
	...
	// The mos/65 protocol introduces proper transaction announcements, so instead
...
func (pm *ProtocolManager) txsyncLoop64() {
	...
	send := func(s *txsync) {
		if s.p.version >= eth65 {
			panic("initial transaction syncer running on mos/65+")
		}
...
func (pm *ProtocolManager) doSync(op *chainSyncOp) error {
	if op.mode == downloader.FastSync {
		...
		// txlookup limit.
		// The main concern here is: during the fast sync Gmos won't index the
		// block(generate tx indices) before the HEAD-limit. But if user changes
		// the limit in the next fast sync(e.g. user kill Gmos manually and
		// restart) then it will be hard for Gmos to figure out the oldest block
		// has been indexed. So here for the user-experience wise, it's non-optimal
		// that user can't change limit during the fast sync. If changed, Gmos
