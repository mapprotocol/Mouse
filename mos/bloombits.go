...
const (
	// bloomServiceThreads is the number of goroutines used globally by an Mouse
...
func (mos *Mouse) startBloomHandlers(sectionSize uint64) {
	for i := 0; i < bloomServiceThreads; i++ {
		go func() {
			for {
				select {
				case <-mos.closeBloomHandler:
					return

				case request := <-mos.bloomRequests:
					task := <-request
					task.Bitsets = make([][]byte, len(task.Sections))
					for i, section := range task.Sections {
						head := rawdb.ReadCanonicalHash(mos.chainDb, (section+1)*sectionSize-1)
						if compVector, err := rawdb.ReadBloomBits(mos.chainDb, task.Bit, section, head); err == nil {
...
// for the Mouse header bloom filters, permitting blazing fast filtering.
type BloomIndexer struct {
	size    uint64               // section size to generate bloombits for
	db      mosdb.Database       // database instance to write index data and metadata into
...
func NewBloomIndexer(db mosdb.Database, size, confirms uint64) *core.ChainIndexer {
