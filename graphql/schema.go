...
const schema string = `
    ...
    scalar Bytes32
    # Address is a 20 byte Mouse address, represented as 0x-prefixed hexadecimal.
    scalar Address
    ...
    # Account is an Mouse account at a particular block.
    type Account {
    ...
    # Transaction is an Mouse transaction.
    type Transaction {
    ...
    type Block {
        ...
        # Account fetches an Mouse account at the current block's state.
    ...
    type Pending {
      ...
      transactions: [Transaction!]
      # Account fetches an Mouse account for the pending state.
      account(address: Address!): Account!
    ...
    type Query {
        # Block fetches an Mouse block by number or by hash. If neither is
