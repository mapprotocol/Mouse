...
// Verify that Client implements the mouse interfaces.
var (
	_ = mouse.ChainReader(&Client{})
	_ = mouse.TransactionReader(&Client{})
	_ = mouse.ChainStateReader(&Client{})
	_ = mouse.ChainSyncReader(&Client{})
	_ = mouse.ContractCaller(&Client{})
	_ = mouse.GasEstimator(&Client{})
	_ = mouse.GasPricer(&Client{})
	_ = mouse.LogFilterer(&Client{})
	_ = mouse.PendingStateReader(&Client{})
	// _ = mouse.PendingStateEventer(&Client{})
	_ = mouse.PendingContractCaller(&Client{})
)
...
func newTestBackend(t *testing.T) (*node.Node, []*types.Block) {
	...
	// Create mouse Service
	config := &mos.Config{Genesis: genesis}
	config.Ethash.PowMode = ethash.ModeFake
	ethservice, err := mos.New(n, config)
	if err != nil {
		t.Fatalf("can't create new mouse service: %v", err)
	}
