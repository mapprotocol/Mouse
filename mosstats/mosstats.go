...
// Package mosstats implements the network stats reporting service.
package mosstats
...
// backend encompasses the bare-minimum functionality needed for mosstats reporting
...
// Service implements an Mouse netstats reporting daemon that pushes local
...
func New(node *node.Node, backend backend, engine consensus.Engine, url string) error {
	...
	node.RegisterLifecycle(mosstats)
	return nil
}
...
func (s *Service) login(conn *connWrapper) error {
	...
	if info := infos.Protocols["mos"]; info != nil {
		network = fmt.Sprintf("%d", info.(*mos.NodeInfo).Network)
		protocol = fmt.Sprintf("mos/%d", mos.ProtocolVersions[0])
	} else {
...
func (s *Service) reportLatency(conn *connWrapper) error {
	// Send the current time to the mosstats server
	start := time.Now()
	...
	log.Trace("Sending measured latency to mosstats", "latency", latency)
...
func (s *Service) reportBlock(conn *connWrapper, block *types.Block) error {
	...
	log.Trace("Sending new block to mosstats", "number", details.Number, "hash", details.Hash)
...
func (s *Service) reportHistory(conn *connWrapper, list []uint64) error {
	...
	if len(history) > 0 {
		log.Trace("Sending historical blocks to mosstats", "first", history[0].Number, "last", history[len(history)-1].Number)
	} else {
...
func (s *Service) reportPending(conn *connWrapper) error {
	...
	log.Trace("Sending pending transactions to mosstats", "count", pending)
...
func (s *Service) reportStats(conn *connWrapper) error {
	...
	log.Trace("Sending node details to mosstats")
