...
type Sync struct {
	database mosdb.KeyValueReader     // Persistent database to check for existing entries
...
func NewSync(root common.Hash, database mosdb.KeyValueReader, callback LeafCallback, bloom *SyncBloom) *Sync {
	ts := &Sync{
		database: database,
...
// both mos/6x style fast sync and snap/1x style state sync, the paths of trie
...
func (s *Sync) Commit(dbw mosdb.Batch) error {
