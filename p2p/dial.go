...
func newDialState(self enode.ID, ntab discoverTable, maxdyn int, cfg *Config, bootstrapNodes []*enode.Node, chainID int) *dialstate {
	s := &dialstate{
		maxDynDials: maxdyn,
		ntab:        ntab,
		self:        self,
		...
		bootnodes:   make([]*enode.Node, len(bootstrapNodes)),
		randomNodes: make([]*enode.Node, maxdyn/2),
		chainType:   chainID,
	}
	copy(s.bootnodes, bootstrapNodes)
	if s.log == nil {
		s.log = log.Root()
	}
...
func (s *dialstate) newTasks(nRunning int, peers map[enode.ID]*Peer, now time.Time, chainType int) []task {
	if s.start.IsZero() {
		s.start = now
	}
	...
	addDial := func(flag connFlag, n *enode.Node) bool {
		...
		s.dialing[n.ID()] = flag
		newtasks = append(newtasks, &dialTask{flags: flag, dest: n, chainType: chainType})
		return true
	}
	...
	if len(s.lookupBuf) < needDynDials && !s.lookupRunning {
		...
		newtasks = append(newtasks, &discoverTask{chainType: chainType})
	}
	...
	if nRunning == 0 && len(newtasks) == 0 && s.hist.Len() > 0 {
		t := &waitExpireTask{s.hist.nextExpiry().Sub(now), chainType}
		newtasks = append(newtasks, t)
	}
	return newtasks
}
...
func (t *dialTask) resolve(srv *Server) bool {
	if srv.ntab == nil || srv.ntabOther == nil {
...
func (t *dialTask) dial(srv *Server, dest *enode.Node) error {
	...
	return srv.SetupConn(mfd, t.flags, dest, t.chainType)
}
...
func (t *waitExpireTask) Type() int {
	return t.chainType
}
