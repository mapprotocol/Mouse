...
func newTester(t *testing.T, confOverride func(*mos.Config)) *tester {
	...
	// Create a networkless protocol stack and start an mouse service within
	...
	ethBackend, err := mos.New(stack, ethConf)
	if err != nil {
		t.Fatalf("failed to register mouse protocol: %v", err)
	}
