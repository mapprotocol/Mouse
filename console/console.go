...
import (
	...
	"github.com/marcopoloprotoco/mouse/internal/jsre"
	"github.com/marcopoloprotoco/mouse/internal/jsre/deps"
	"github.com/marcopoloprotoco/mouse/internal/web3ext"
...
type Config struct {
	...
	Client   *rpc.Client         // RPC client to execute Mouse requests through
...
func (c *Console) initExtensions() error {
	...
	aliases := map[string]struct{}{"mos": {}, "personal": {}}
	for api := range apis {
		if api == "web3" {
			continue
		}
...
func (c *Console) AutoCompleteInput(line string, pos int) (string, []string, string) {
	...
	// E.g. in case of nested lines mos.getBalance(mos.coinb<tab><tab>
	start := pos - 1
	for ; start > 0; start-- {
...
// Welcome show summary of current Gmos instance and some metadata about the
// console's available modules.
func (c *Console) Welcome() {
	message := "Welcome to the Gmos JavaScript console!\n\n"

	// Print some generic Gmos metadata
	if res, err := c.jsre.Run(`
		var message = "instance: " + web3.version.node + "\n";
		try {
			message += "coinbase: " + mos.coinbase + "\n";
		} catch (err) {}
		message += "at block: " + mos.blockNumber + " (" + new Date(1000 * mos.getBlock(mos.blockNumber).timestamp) + ")\n";
		try {
