...
const (
	...
	ethVersion        = 63              // equivalent mos version for the downloader
...
func newServerHandler(server *LesServer, blockchain *core.BlockChain, chainDb mosdb.Database, txpool *core.TxPool, synced func() bool) *serverHandler {
...
func (h *serverHandler) handle(p *clientPeer) error {
	...
	for {
		select {
		case err := <-p.errCh:
			p.Log().Debug("Failed to send light mouse response", "err", err)
			return err
		default:
		}
