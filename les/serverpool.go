...
import (
	...
	"github.com/marcopoloprotoco/mouse/p2p/nodestate"
...
func newServerPool(db mosdb.KeyValueStore, dbKey []byte, vt *lpc.ValueTracker, discovery enode.Iterator, mixTimeout time.Duration, query queryFunc, clock mclock.Clock, trustedURLs []string) *serverPool {
	s := &serverPool{
		db:           db,
		clock:        clock,
