...
func SetupGenesisBlock(db mosdb.Database, genesis *Genesis) (*params.ChainConfig, common.Hash, error) {
...
func (g *Genesis) ToBlock(db mosdb.Database) *types.Block {
	if db == nil {
...
func (g *Genesis) Commit(db mosdb.Database) (*types.Block, error) {
	block := g.ToBlock(db)
...
// The block is committed as the canonical head block.
func (g *Genesis) MustCommit(db mosdb.Database) *types.Block {
...
func GenesisBlockForTesting(db mosdb.Database, addr common.Address, balance *big.Int) *types.Block {
...
// DeveloperGenesisBlock returns the 'gmos --dev' genesis block.
