...
func InitDatabaseFromFreezer(db mosdb.Database) {
...
func iterateTransactions(db mosdb.Database, from uint64, to uint64, reverse bool) (chan *blockTxHashes, chan struct{}) {
...
func IndexTransactions(db mosdb.Database, from uint64, to uint64) {
...
func UnindexTransactions(db mosdb.Database, from uint64, to uint64) {
	// short circuit for invalid range
	if from >= to {
		return
	}
