...
// SnapshotTree is an Mouse state snapshot tree. It consists of one persistent
...
type Tree struct {
	diskdb mosdb.KeyValueStore      // Persistent database to store the snapshot
...
// background thread.
func New(diskdb mosdb.KeyValueStore, triedb *trie.Database, cache int, root common.Hash, async bool) *Tree {
