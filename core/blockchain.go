...
// Package core implements the Mouse consensus protocol.
package core
...
type BlockChain struct {
	...
	db     mosdb.Database // Low level persistent database to store final content in
...
// available in the database. It initialises the default Mouse Validator and
// Processor.
func NewBlockChain(db mosdb.Database, cacheConfig *CacheConfig, chainConfig *params.ChainConfig, engine consensus.Engine, vmConfig vm.Config, shouldPreserve func(block *types.Block) bool, txLookupLimit *uint64) (*BlockChain, error) {
	if cacheConfig == nil {
...
func (bc *BlockChain) SetHead(head uint64) error {
	...
	updateFn := func(db mosdb.KeyValueWriter, header *types.Header) (uint64, bool) {
	...
	delFn := func(db mosdb.KeyValueWriter, hash common.Hash, num uint64) {
	...
	if bc.cacheConfig.TrieDirtyDisabled {
		...
		if current := block.NumberU64(); current > TriesInMemory {
			...
			if nodes > limit || imgs > 4*1024*1024 {
				triedb.Cap(limit - mosdb.IdealBatchSize)
			}
